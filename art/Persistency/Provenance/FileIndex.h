...
namespace edm {
  ...
  class FileIndex {
    ...
    public:
      ...
      void addEntry(RunNumber_t run, SubRunNumber_t lumi, EventNumber_t event, EntryNumber_t entry);
	  ...
	  static EntryNumber_t const invalidEntry = -1LL;
          ...
          Element(RunNumber_t run, SubRunNumber_t lumi, EventNumber_t event, long long entry) :
            run_(run), lumi_(lumi),
          event_(event), entry_(entry) {
	    assert(lumi_ != 0U || event_ == 0U);
	  }
          Element(RunNumber_t run, SubRunNumber_t lumi, EventNumber_t event) :
            run_(run), lumi_(lumi), event_(event), entry_(invalidEntry) {}
          EntryType getEntryType() const {
	    return lumi_ == 0U ? kRun : (event_ == 0U ? kLumi : kEvent);
          }
          RunNumber_t run_;
          SubRunNumber_t lumi_;
      ...
      const_iterator
      findPosition(RunNumber_t run, SubRunNumber_t lumi = 0U, EventNumber_t event = 0U) const;

      const_iterator
      findEventPosition(RunNumber_t run, SubRunNumber_t lumi, EventNumber_t event, bool exact) const;

      const_iterator
      findLumiPosition(RunNumber_t run, SubRunNumber_t lumi, bool exact) const;

      const_iterator
      findRunPosition(RunNumber_t run, bool exact) const;

      const_iterator
      findLumiOrRunPosition(RunNumber_t run, SubRunNumber_t lumi) const;

      bool
      containsEvent(RunNumber_t run, SubRunNumber_t lumi, EventNumber_t event, bool exact) const {
	return findEventPosition(run, lumi, event, exact) != entries_.end();
      }

      bool
      containsLumi(RunNumber_t run, SubRunNumber_t lumi, bool exact) const {
