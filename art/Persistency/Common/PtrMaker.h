...
namespace art {
  ...
  template <typename T>
  concept can_call_productGetter = requires(T& t, ProductID const pid) {
  ...
  public:
    ...
    // other modules.
    template <can_call_productGetter DataLevel>
  ...
  {
    auto const pid = evt.template getProductID<Container>(instance);
