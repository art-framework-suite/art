...
namespace {
  ...
  struct three_in_container{
    ...
    void insert(iterator self_end, iterator other_beg, iterator other_end){
      (void) self_end;
      (void) other_beg;
      (void) other_end;
    }
...
TEST_CASE("concatContainers() on valid custom containers"){
  CHECK(can_concat<two_in_container, two_in_container>); //Checking with two arg insert
  CHECK(can_concat<three_in_container, three_in_container>); //Checking with three arg insert
}
