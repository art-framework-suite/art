...
namespace {
  ...
  makeProcessConfiguration(std::string const& process_name,
                           std::string const& module_label)
  {
    ...
    return ProcessConfiguration{
      process_name, processParams.id(), getReleaseVersion()};
  }
} // namespace
...
BOOST_AUTO_TEST_CASE(concurrent_insertion_reading)
{
  ...
  auto makeProcessHistory = [&pcs](std::size_t const first,
