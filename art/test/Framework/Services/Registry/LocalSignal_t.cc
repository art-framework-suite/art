...
namespace {
  ...
  using TestSignal2 = art::LocalSignal<art::detail::SignalResponseType::FIFO, void(std::ostream&, std::string const&)>;
  using TestSignal2a = art::LocalSignal<art::detail::SignalResponseType::LIFO, void(std::ostream&, std::string const&)>;
  using TestSignal1 = art::LocalSignal<art::detail::SignalResponseType::FIFO, void(std::ostream&)>;
  using TestSignal0 = art::LocalSignal<art::detail::SignalResponseType::FIFO, void()>;
...
BOOST_AUTO_TEST_CASE(TestSignal1_All_t)
{
  ...
  std::string const test_text {"Test text"};
  boost::test_tools::output_test_stream os;
  BOOST_CHECK_NO_THROW(s.watchAll([&test_text](auto& x){ testCallback<0>(x, test_text); }));
  BOOST_CHECK_NO_THROW(s.invoke(sID, os));
  BOOST_CHECK(os.is_equal(test_text));
}
...
BOOST_AUTO_TEST_CASE(TestSignal0_All_t)
{
  ...
  std::string const test_text {"Test text"};
  boost::test_tools::output_test_stream os;
  BOOST_CHECK_NO_THROW(s.watchAll([&os, &test_text]{ testCallback<0>(os, test_text); }));
  BOOST_CHECK_NO_THROW(s.invoke(sID));
  BOOST_CHECK(os.is_equal(test_text));
}
