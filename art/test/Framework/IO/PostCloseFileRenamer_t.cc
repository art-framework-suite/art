...
namespace {
  ...
  TestFixture::TestFixture() : fstats("label", "DEVEL") {}
...
BOOST_AUTO_TEST_CASE(SeqNo)
{
  ...
  BOOST_CHECK_EQUAL(fr.applySubstitutions(pattern),
                    std::string("ethel-00-charlie"));
  fstats.recordFileOpen();
  BOOST_CHECK_EQUAL(fr.applySubstitutions(pattern),
                    std::string("ethel-00-charlie"));
  fstats.recordFileClose();
  BOOST_CHECK_EQUAL(fr.applySubstitutions(pattern),
                    std::string("ethel-01-charlie"));
  fstats.recordFileOpen();
  BOOST_CHECK_EQUAL(fr.applySubstitutions(pattern),
                    std::string("ethel-01-charlie"));
  fstats.recordFileClose();
  BOOST_CHECK_EQUAL(fr.applySubstitutions(pattern),
                    std::string("ethel-02-charlie"));
}
...
BOOST_AUTO_TEST_CASE(SimpleRegex)
{
  ...
  BOOST_CHECK_EQUAL(fr.applySubstitutions(pattern), std::string("/tmp/c.dat"));
}
...
BOOST_AUTO_TEST_CASE(gFlagRegex)
{
  ...
  PostCloseFileRenamer fr(fstats);
  fstats.recordInputFile("/tmp/c.root");
  BOOST_CHECK_EQUAL(fr.applySubstitutions(pattern), std::string("/tmp/c.rfft"));
}
