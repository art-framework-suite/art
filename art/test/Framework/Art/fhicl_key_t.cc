...
{
  BOOST_CHECK_EQUAL(fhicl_key("a"), "a"s);
  BOOST_CHECK_EQUAL(fhicl_key("b"s), "b"s);
}

BOOST_AUTO_TEST_CASE(multiple_args)
{
  BOOST_CHECK_EQUAL(fhicl_key("a", "b"), "a.b"s);
  BOOST_CHECK_EQUAL(fhicl_key("a", "b", "c"), "a.b.c"s);
  auto const& stem = fhicl_key("a", "b");
  BOOST_CHECK_EQUAL(fhicl_key(stem, "c"), "a.b.c"s);
  BOOST_CHECK_EQUAL(fhicl_key("", "a"), "a"s);
}
