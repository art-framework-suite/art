...
namespace {
  ...
  using logged_memory_t = std::vector<std::tuple<sid_size_type,vsize_t, rss_t>>;
  ...
  public:
    ...
    void update(sid_size_type const sid, logged_memory_t& loggedMemory)
    {
  ...
  void test(ProcInfoTester& procInfo, sid_size_type const sid, logged_memory_t& data)
  {
...
int main()
{
  ...
  for (sid_size_type i {}; i < nSchedules; ++i) {
    tasks.push_back([&procInfo,i,&loggedMemory]{ test(procInfo, i, loggedMemory); });
  }
  ...
  std::partition_copy(loggedMemory.cbegin(), loggedMemory.cend(),
                      std::back_inserter(schedule1), std::back_inserter(schedule2),
                      [](auto const& t){
