...
namespace {
  ...
  struct TagBuilder {
    TagBuilder(std::string const& label,
               std::string const& process)
      : label_{label}
      , process_{process}
    {}

    art::InputTag operator()(std::string const& instance)
    {
  ...
  struct Config {
    fhicl::TupleAs<TagBuilder(std::string,std::string)>
    tagInfo { fhicl::Name("tagInfo"), fhicl::Comment("First argument is the module label.\n"
...
namespace arttest {
  ...
  public:
    ...
    void beginSubRun(art::SubRun const& sr) override
    {
      ...
      auto const& denomH = sr.getValidHandle<unsigned>(tagInfo_("bgnSubRunDenom"));
      BOOST_CHECK(art::same_ranges(numH, denomH));
    ...
    void endSubRun(art::SubRun const& sr) override
    {
      ...
      auto const& denomH = sr.getValidHandle<unsigned>(tagInfo_("endSubRunDenom"));
      BOOST_CHECK(art::same_ranges(numH, denomH));
      BOOST_CHECK(!numH.provenance()->rangeOfValidity().is_full_subRun());
    }
