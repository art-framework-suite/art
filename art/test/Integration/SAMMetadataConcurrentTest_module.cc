...
namespace {
  ...
  private:
    ...
    void beginJob() override
    {
      ...
      cet::transform_all(mockMD,
                         back_inserter(tasks),
                         [&md](auto const& pr) {
                           return [&md, &pr]{
                             md->addMetadataString(pr.first, pr.second);
                           };
                         });
    ...
    void endJob() override {
      ...
      auto file_type_it = cet::find_in_all(coll, value_type{"file_type", cet::canonical_string("unknown")});

      auto const& process_name = art::ServiceHandle<art::TriggerNamesService const>{}->getProcessName();
      auto process_name_it = cet::find_in_all(coll, value_type{"process_name", cet::canonical_string(process_name)});
      ...
      for (auto c = coll.cbegin(), m = mockMD.cbegin(); c != coll.cend(); ++c, ++m) {
