...
public:
  ...
  void
  update_subRun(OutputFileStatus const status);

  void
  update_run(OutputFileStatus const status);
...
private:
  ...
  std::array<unsigned, Granularity::NBoundaries()>
  counts_{{}};
...
public:
  ...
  struct Defaults {
    ...
    static constexpr unsigned unsigned_max() { return std::numeric_limits<unsigned>::max(); }
    static constexpr unsigned size_max() { return 0x7f000000u; }
    static constexpr unsigned seconds_max() { return std::chrono::duration_values<unsigned>::max(); }
    static constexpr char const* granularity_default() { return "Event"; }
  ...
  struct Config : Defaults {
    ...
    granularity{fhicl::Name("granularity"), Comment(
  ...
  bool
  should_close(FileProperties const&) const;
...
private:
  ...
  Granularity
  granularity_{Granularity::value(ClosingCriteria::Defaults::granularity_default())};
