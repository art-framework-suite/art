...
FileProperties::
FileProperties(unsigned const events, unsigned const subRuns, unsigned const runs, unsigned const inputFiles, unsigned const the_size, seconds_t const the_age)
...
ClosingCriteria::
ClosingCriteria(Config const& c)
  : ClosingCriteria{FileProperties{c.maxEvents(), c.maxSubRuns(), c.maxRuns(), c.maxInputFiles(), c.maxSize(), seconds_t{c.maxAge()}}, c.granularity()}
{
...
bool
ClosingCriteria::
should_close(FileProperties const& fp) const
{
