...
int main( int argc, char** argv ){
  ...
  desc.add_options()
    ...
    ("fraction,f",  bpo::value<double>()->default_value(0.05,"0.05"),
  ...
  if ( vm.count("fraction") ) {
    ...
    if (minimumFraction < 0 || minimumFraction > 1) {
      std::cerr << "Must choose fraction between 0 and 1\n"
                << "For usage and options list, please do 'product_sizes_dumper --help'.\n";
      return 4;
    }
  }
  ...
  for ( auto const& filename : file_names ) {
    ...
    if ( file_names.size() > 1 && n!=0 ) std::cout << separator << std::endl;
    ...
    if ( file_names.size() > 1 ) std::cout << "Done: " << filename << std::endl;
