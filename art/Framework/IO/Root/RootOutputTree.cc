...
static
void
fillTreeBranches(TTree* /*tree*/, vector<TBranch*> const& branches,
                 bool saveMemory, int64_t threshold)
{
  ...
  ///if (((string(tree->GetName()) == "RunMetaData") || (string(tree->GetName()) == "SubRunMetaData")) && (branches.size() != 0)) {
  ///  cout << "-----> RootOutputTree::fillTreeBranches(...) ... Entry " << branches.front()->GetEntryNumber() << " filled." << endl;
  ///}
}
