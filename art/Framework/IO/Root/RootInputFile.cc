...
void
RootInputFile::RootInputTree::
addBranch(BranchKey const& key, BranchDescription const& bd, string const& branchName, bool const present)
{
  assert(isValid());
  ...
  , saveMemoryObjectThreshold_{saveMemoryObjectThreshold}
{
  ...
  // Read the ProductList
  {
    ...
    productListHolder_ = make_unique<ProductRegistry>(move(productList));
  }
...
createFileBlock()
{
  return make_unique<FileBlock>(fileFormatVersion_, eventTree().tree(), fastClonable(), fileName_, move(branchChildren_),
                                readResults());
}
...
unique_ptr<SubRunPrincipal>
RootInputFile::
readCurrentSubRun(EntryNumbers const& entryNumbers, cet::exempt_ptr<RunPrincipal> rp [[gnu::unused]])
{
...
fillPerBranchTypePresenceFlags(ProductList const& prodList)
{
  for (auto const& bk_and_bd : prodList) {
    ...
    if (treePointers_[bt]->tree()->GetBranch(bd.branchName().c_str()) != nullptr) {
      perBranchTypeProdPresence_[bt].emplace(pid);
    }
  }
}
...
readResults()
{
  ...
  if (!resultsTree().isValid()) {
    resp = make_unique<ResultsPrincipal>(ResultsAuxiliary{}, processConfiguration_);
    return resp;
  }
  EntryNumbers const& enumbers{0};
  fillAuxiliary_Results(0);
