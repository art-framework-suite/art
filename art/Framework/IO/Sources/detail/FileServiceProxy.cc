...
FileServiceProxy(std::vector<std::string> && fileNames,
                 size_t attempts,
...
obtainURI_()
{
  ...
  case FileDeliveryStatus::PENDING:
    ...
    currentItem_.uriStatus = static_cast<FileDeliveryStatus>(ci_->getNextFileURI(currentItem_.uri, wait));
    return obtainURI_();
...
obtainFileFromURI_()
{
  ...
  case FileTransferStatus::PENDING:
    if (! currentItem_.attemptsRemaining--) {
      throw Exception(errors::CatalogServiceError)
        << "Unable to obtain URI "
        << currentItem_.uri
        << " from FileTransfer service after "
        << attemptsPerPhase_
        << " attempts.\n";
    }
    currentItem_.ftStatus =
      static_cast<FileTransferStatus>
  ...
  default:
    throw Exception(errors::CatalogServiceError)
      ...
      << " ("
      << static_cast<int>(currentItem_.ftStatus)
      <<") for URI "
      << currentItem_.uri
      << ".\n";
  }
