...
template <typename T, typename P>
std::enable_if_t<P::branch_type == art::InEvent ||
                 P::branch_type == art::InResults>
                              ...
                              std::string const& instance_name)
{
  ...
  BranchDescription const& desc =
    get_ProductDescription<T>(principal, module_label, instance_name);
  ...
  principal.put(std::move(wp),
                ...
                std::make_unique<ProductProvenance const>(
                  desc.productID(), productstatus::present()));
}

template <typename T, typename P>
std::enable_if_t<P::branch_type == art::InSubRun ||
                 P::branch_type == art::InRun>
art::put_product_in_principal(std::unique_ptr<T>&& product,
                              P& principal,
                              std::string const& module_label,
                              std::string const& instance_name,
