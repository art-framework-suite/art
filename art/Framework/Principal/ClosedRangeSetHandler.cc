...
constexpr
auto
invalid_eid = IDNumber<Level::Event>::invalid();
...
ClosedRangeSetHandler::EventInfo::
EventInfo(EventInfo const& rhs) noexcept
  : id_{rhs.id_}
...
ClosedRangeSetHandler::EventInfo::
EventInfo(EventInfo&& rhs) noexcept
  : id_{move(rhs.id_)}
...
ClosedRangeSetHandler::EventInfo&
ClosedRangeSetHandler::EventInfo::
operator=(EventInfo const& rhs) noexcept
{
  if (this != &rhs) {
    id_ = rhs.id_;
    lastInSubRun_ = rhs.lastInSubRun_;
...
ClosedRangeSetHandler::EventInfo&
ClosedRangeSetHandler::EventInfo::
operator=(EventInfo&& rhs) noexcept
{
  id_ = move(rhs.id_);
  lastInSubRun_ = move(rhs.lastInSubRun_);
  return *this;
...
// either!
ClosedRangeSetHandler::
ClosedRangeSetHandler(RangeSet const& rs)
  : ranges_{rs}
  , idx_{0}
  , eventInfo_{}
{
...
// either!
ClosedRangeSetHandler::
ClosedRangeSetHandler(ClosedRangeSetHandler const& rhs)
  : ranges_{rhs.ranges_}
  , idx_{rhs.idx_}
...
// either!
ClosedRangeSetHandler::
ClosedRangeSetHandler(ClosedRangeSetHandler&& rhs)
...
ClosedRangeSetHandler&
ClosedRangeSetHandler::
operator=(ClosedRangeSetHandler const& rhs)
{
  if (this != &rhs) {
    ranges_ = rhs.ranges_;
    idx_ = rhs.idx_;
...
ClosedRangeSetHandler&
ClosedRangeSetHandler::
operator=(ClosedRangeSetHandler&& rhs)
{
  ranges_ = move(rhs.ranges_);
  idx_ = move(rhs.idx_);
  eventInfo_ = move(rhs.eventInfo_);
...
do_update(EventID const& id, bool const lastInSubRun)
{
  ...
  while (idx_ != end_idx() && !ranges_.at(idx_).contains(id.subRun(), id.event())) {
    ++idx_;
  }
}
...
do_maybeSplitRange()
{
  if (!eventInfo_.lastInSubRun() && (idx_ != end_idx())) {
    auto split_range = ranges_.split_range(eventInfo_.id().subRun(), eventInfo_.id().next().event());
