...
cet::exception&
exceptionContext(ModuleDescription const& md, Principal const& principal, cet::exception& e)
{
  e << md.moduleName()
    << "/"
    << md.moduleLabel()
...
Worker::
Worker(ModuleDescription const& md, WorkerParams const& wp)
  : md_{md}
  ...
  , counts_thrown_{}
{
  TDEBUG(5) << "Worker ctor: 0x" << hex << ((unsigned long)this) << dec << " (" << wp.streamIndex_ << ")" << " name: " << md.moduleName() << " label: " << md.moduleLabel() << "\n";
}
...
reset(int si)
{
  ...
  {
    ostringstream buf;
    buf << "-----> Worker::reset: 0x" << hex << ((unsigned long)this) << dec << " Resetting waitingTasks_ (" << si << ") ...\n";
    TDEBUG(6) << buf.str();
  }
  waitingTasks_.reset();
  workStarted_ = false;
...
{
  LogError("BeginJob")
      << "A bad_alloc is going through " << workerType() << ":\n"
      << md_ << "\n";
  throw;
}
catch (exception& e)
...
{
  LogError("BeginJob")
      << "module caught an string during beginJob\n";
...
{
  LogError("EndJob")
      << "An exception is going through " << workerType() << ":\n"
      << md_ << "\n";
  throw Exception(errors::StdException)
      << "A exception is going through " << workerType() << ":\n"
      << md_ << "\n" << e.what();
...
{
  LogError("EndJob")
      << "module caught an string during endJob\n";
  throw Exception(errors::BadExceptionType)
      << "string = " << s << "\n"
      << md_ << "\n";
}
...
bool
Worker::
doWork(Transition trans, Principal& principal, CurrentProcessingContext* cpc)
{
  switch (state_) {
    case Ready:
      break;
  ...
  try {
    ...
    state_ = ExceptionThrown;
    auto art_ex = Exception{errors::BadAlloc}
    ...
    auto art_ex = Exception{errors::BadExceptionType, "string"}
      << "A string thrown as an exception occurred during a call to the module ";
    cached_exception_ = make_exception_ptr(art_ex);
    detail::exceptionContext(md_, principal, art_ex)
        << "and cannot be repropagated.\n"
        << "Previous information:\n string = " << s;
...
void
Worker::
doWork_event(EventPrincipal& p, int si, CurrentProcessingContext* cpc)
{
  ++counts_visited_;
  returnCode_ = false;
  try {
    ...
    if (action == actions::IgnoreCompletely) {
      ...
      if (auto edmEx = dynamic_cast<Exception*>(&e)) {
        ...
        cached_exception_ = make_exception_ptr(Exception{errors::OtherArt, string(), e});
      }
      rethrow_exception(cached_exception_);
    }
  }
...
void
Worker::
doWork_event(WaitingTask* workerInPathDoneTask, EventPrincipal& p, int si, CurrentProcessingContext* cpc)
{
  ...
  if (workStarted_.compare_exchange_strong(expected, true)) {
      ...
      try {
        ...
        if (action == actions::IgnoreCompletely) {
          ...
          if (auto edmEx = dynamic_cast<Exception*>(&e)) {
            ...
            This->cached_exception_ = make_exception_ptr(Exception{errors::OtherArt, string(), e});
          }
          This->waitingTasks_.doneWaiting(cached_exception_);
          TDEBUG(4) << "=====> End   runWorkerFunctor (" << si << ") ... because of exception\n";
          return;
        }
      }
      catch (bad_alloc const& bda) {
    ...
    if (chain) {
      ...
      TDEBUG(4) << "-----> Worker::doWork_event: si: " << si << " pushing onto chain " << hex << ((unsigned long*)chain) << dec << "\n";
      chain->push(runWorkerFunctor);
      TDEBUG(4) << "-----> End   Worker::doWork_event (" << si << ") ...\n";
      return;
    }
    // Must be a stream or global module.
    TDEBUG(4) << "-----> Worker::doWork_event: si: " << si << " calling worker functor\n";
    runWorkerFunctor();
    TDEBUG(4) << "-----> End   Worker::doWork_event (" << si << ") ...\n";
    return;
  }
  ...
  TDEBUG(4) << "-----> End   Worker::doWork_event (" << si << ") ... work already in progress on another path\n";
}
