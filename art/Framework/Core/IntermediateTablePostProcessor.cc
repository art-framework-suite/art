...
{
  applyRethrowOptions(raw_config);
  // process_name
...
void
art::IntermediateTablePostProcessor::
applyRethrowOptions(intermediate_table & raw_config) const
{
  ...
  raw_config["services.scheduler.IgnoreCompletely"] =
    extended_value(false, SEQUENCE, sequence_t());
  raw_config["services.scheduler.SkipEvent"] =
    extended_value(false, SEQUENCE, sequence_t());
  raw_config["services.scheduler.FailModule"] =
    extended_value(false, SEQUENCE, sequence_t());
  raw_config["services.scheduler.FailPath"] =
    extended_value(false, SEQUENCE, sequence_t());
}
