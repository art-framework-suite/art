...
namespace art {
  ...
  public:
    ...
    modifiesEvent() const
    {
      return worker_->modifiesEvent();
    }

  private:
    using Counts_t = ExecutionCounts<stats::Visited,
                                     stats::Passed,
                                     stats::Failed,
  ...
  template <typename T>
  bool
  WorkerInPath::runWorker(typename T::MyPrincipal& ep,
                          CurrentProcessingContext const* cpc)
  {
    ...
    if (T::level == Level::Event && filterAction_ == Veto)
      rc = !rc;
    else if (T::level != Level::Event || filterAction_ == Ignore)
      rc = true;
