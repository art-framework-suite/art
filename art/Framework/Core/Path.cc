...
void
Path::
process(Transition trans, Principal& principal)
{
  ...
  std::size_t idx = 0;
  for (auto I = workers_.begin(), E = workers_.end(); (I != E) && should_continue; ++I, ++idx) {
    try {
...
void
Path::
process_event_for_endpath(EventPrincipal& ep, int si)
{
  ...
  for (; should_continue && (idx < max_idx); ++idx) {
    ...
    try {
      ...
      if (action != actions::FailPath) {
        ...
        TDEBUG(4) << "-----> End   Path::process_event_for_endpath (" << si << ") ... terminate path because of EXCEPTION\n";
        throw Exception{errors::ScheduleExecutionFailure, "Path: ProcessingStopped.", e}
            << "Exception going through path " << name_ << "\n";
      }
      ...
      TDEBUG(4) << "-----> End   Path::process_event_for_endpath (" << si << ") ... terminate end path because of EXCEPTION\n";
      return;
    }
    // Note: This will only be set false by a filter which has rejected (impossible on the end path!).
    should_continue = workerInPath.returnCode(si);
    TDEBUG(5) << "-----> process_event_for_endpath: si: " << si << " idx: " << idx << " should_continue: " << should_continue << "\n";
  }
  ...
  try {
    ...
    TDEBUG(4) << "-----> End   Path::process_event_for_endpath (" << si << ") ... terminate end path final processing because of EXCEPTION\n";
    throw;
  }
...
void
Path::
process_event(WaitingTask* pathsDoneTask, EventPrincipal& ep, int si)
{
  TDEBUG(4) << "-----> Begin Path::process_event (" << si << ") ...\n";
  {
    ostringstream buf;
    buf << "-----> Path::process_event: 0x" << hex << ((unsigned long)this) << dec << " Resetting waitingTasks_ (" << si << ") ...\n";
    TDEBUG(6) << buf.str();
  }
...
void
Path::
process_event_idx_asynch(size_t idx, size_t const max_idx, EventPrincipal& ep, int si, bool should_continue,
                         CurrentProcessingContext* cpc)
{
  TDEBUG(4) << "-----> Begin Path::process_event_idx_asynch: si: " << si << " idx: " << idx << " max_idx: " << max_idx << " ...\n";
  auto runWorkerTaskFunctor = [this, idx, max_idx, &ep, si, should_continue, cpc](std::exception_ptr const* /*unused*/) {
    ...
    auto new_cpc = cpc;
    try {
      process_event_idx(new_idx, max_idx, ep, si, new_should_continue, new_cpc);
    }
    catch (...) {
      waitingTasks_.doneWaiting(current_exception());
      // End this task, terminating the path here.
      TDEBUG(4) << "=====> End   runWorkerTask (" << si << ") ... path terminate because of exception\n";
      return;
    }
    TDEBUG(4) << "=====> End   runWorkerTask (" << si << ") ...\n";
  };
  auto runWorkerTask = make_waiting_task(tbb::task::allocate_root(), runWorkerTaskFunctor);
  ...
  TDEBUG(4) << "-----> End   Path::process_event_idx_asynch: si: " << si << " idx: " << idx << " max_idx: " << max_idx << " ...\n";
}
...
void
Path::
process_event_idx(size_t const idx, size_t const max_idx, EventPrincipal& ep, int si, bool const should_continue, CurrentProcessingContext* cpc)
{
  TDEBUG(4) << "-----> Begin Path::process_event_idx: si: " << si << " idx: " << idx << " max_idx: " << max_idx << " ...\n";
  auto workerDoneFunctor = [this, idx, max_idx, &ep, si, should_continue, cpc](exception_ptr const* ex) mutable {
    ...
    TDEBUG(4) << "=====> workerDoneTask: si: " << si << " new_should_continue: " << new_should_continue << "\n";
    if (ex != nullptr) {
      try {
        ...
        if (action != actions::FailPath) {
          ...
          auto art_ex = art::Exception{errors::ScheduleExecutionFailure, "Path: ProcessingStopped.", e}
            << "Exception going through path " << name_ << "\n";
          auto ex_ptr = make_exception_ptr(art_ex);
          waitingTasks_.doneWaiting(ex_ptr);
          TDEBUG(4) << "=====> End   workerDoneTask (" << si << ") ... terminate path because of exception\n";
          return;
        }
    ...
    process_event_workerFinished(idx, max_idx, ep, si, new_should_continue, cpc);
    TDEBUG(4) << "=====> End   workerDoneTask ...\n";
    return;
  };
  auto workerDoneTask = make_waiting_task(tbb::task::allocate_root(), workerDoneFunctor);
  auto& workerInPath = workers_[idx];
  cpc->activate(idx, workerInPath.getWorker()->descPtr());
  workerInPath.runWorker_event(workerDoneTask, ep, si, cpc);
  TDEBUG(4) << "-----> End   Path::process_event_idx: si: " << si << " idx: " << idx << " max_idx: " << max_idx << " ...\n";
}
                             ...
                             CurrentProcessingContext* cpc)
{
  ...
  if (should_continue && (new_idx < max_idx)) {
    ...
    // And end this one.
    TDEBUG(4) << "-----> End   Path::process_event_workerFinished: si: " << si << " new_idx: " << new_idx << " max_idx: " << max_idx << "\n";
    return;
  }
  // All done, or filter rejected, or error.
...
void
Path::
process_event_pathFinished(size_t const idx, EventPrincipal& /*ep*/, int si, bool const should_continue, CurrentProcessingContext* /*cpc*/)
{
  TDEBUG(4) << "-----> Begin Path::process_event_pathFinished: si: " << si << " idx: " << idx << " should_continue: " << should_continue << " ...\n";
  try {
    if (should_continue) {
      ++timesPassed_;
      state_ = art::hlt::Pass;
    ...
    waitingTasks_.doneWaiting(current_exception());
    TDEBUG(4) << "-----> End   Path::process_event_pathFinished: si: " << si << " idx: " << idx << " should_continue: " << should_continue << " ...\n";
    return;
  }
