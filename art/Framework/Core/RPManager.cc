...
namespace {
  std::size_t
  countProducers(art::RPManager::RPMap_t const& rpmap)
  {
...
namespace {
             ...
             std::string& errMsg)
  {
    ...
    if (ptype == cet::PluginTypeDeducer<art::ResultsProducer>::value) {
      result = pf.makePlugin<std::unique_ptr<art::RPWorker>,
                             art::RPParams const&,
                             fhicl::ParameterSet const&>(
        libspec, {pconfig.id(), libspec, pkey}, pconfig);
    } else {
      errMsg += +"Parameter set " + omLabel,
        +".results." + pkey + " specifies a plugin " + libspec +
...
auto
art::RPManager::makeRPs_(fhicl::ParameterSet const& ps) -> decltype(rpmap_)
{
  ...
  if (producer_keys.empty()) {
    ...
    auto const path_keys_end =
      std::remove(results_keys.begin(), results_keys.end(), "producers");
    auto const& all_labels = producers.get_names();
    std::set<std::string> all_labels_set(all_labels.cbegin(),
                                         all_labels.cend());
