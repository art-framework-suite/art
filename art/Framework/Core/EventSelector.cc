...
EventSelector::
EventSelector(vector<string> const& pathspecs, vector<string> const& names)
{
  init(pathspecs, names);
}

EventSelector::
EventSelector(vector<string> const& pathspecs)
...
EventSelector::
EventSelector(fhicl::ParameterSet const& config, vector<string> const& triggernames)
{
...
void
EventSelector::
init(vector<string> const& paths, vector<string> const& triggernames)
{
  accept_all_ = false;
  ...
  for (string const& pathSpecifier : paths) {
    ...
    vector<vector<string>::const_iterator> matches = regexMatch(triggernames, realname);
    if (matches.empty()) {
      if (is_glob(realname)) {
        mf::LogWarning("Configuration")
            ...
            << realname
            << " (from trigger-path specification: "
            << pathSpecifier
            << ") \n";
      }
      else {
    ...
    if (!negative_criterion && !noex_demanded && !exception_spec) {
      cet::transform_all(matches, back_inserter(absolute_acceptors_), makeBitInfoPass);
    }
    else if (!negative_criterion && noex_demanded) {
      cet::transform_all(matches, back_inserter(conditional_acceptors_), makeBitInfoPass);
    }
    else if (exception_spec) {
      cet::transform_all(matches, back_inserter(exception_acceptors_), makeBitInfoPass);
    }
    else if (negative_criterion && !noex_demanded) {
      if (matches.empty()) {
        throw art::Exception(errors::Configuration)
...
bool
EventSelector::
selectionDecision(HLTGlobalStatus const& tr) const
{
  if (accept_all_) {
    return true;
  }
...
bool
EventSelector::
acceptOneBit(vector<BitInfo> const& bits, HLTGlobalStatus const& tr, hlt::HLTState const& s) const
{
  bool lookForException = (s == hlt::Exception);
  for (auto const& b : bits) {
    hlt::HLTState const
    bstate = lookForException ? hlt::Exception
...
bool
EventSelector::
acceptAllBits(vector<BitInfo> const& bits, HLTGlobalStatus const& tr) const
{
  for (auto const& b : bits) {
...
bool
EventSelector::
containsExceptions(HLTGlobalStatus const& tr) const
{
...
bool
EventSelector::
identical(vector<bool> const& a, vector<bool> const& b)
{
...
bool
EventSelector::
identical(EventSelector const& a, EventSelector const& b, unsigned int N)
{
...
vector<bool>
EventSelector::
expandDecisionList(vector<BitInfo> const& b, bool PassOrFail, unsigned int n)
{
...
bool
EventSelector::
overlapping(vector<bool> const& a, vector<bool> const& b)
{
  if (a.size() != b.size()) {
    return false;
  }
...
bool
EventSelector::
subset(vector<bool> const& a, vector<bool> const& b)
{
  if (a.size() != b.size()) {
    return false;
  }
...
vector<bool>
EventSelector::
combine(vector<bool> const& a, vector<bool> const& b)
{
