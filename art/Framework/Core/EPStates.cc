...
namespace statemachine {
   namespace {
      art::RunNumber_t INVALID_RUN() {
         static art::RunNumber_t invalid_run = art::RunID().run();
         return invalid_run;
      }
      art::SubRunNumber_t INVALID_SUBRUN() {
         static art::SubRunNumber_t invalid_subRun = art::SubRunID().subRun();
         return invalid_subRun;
      }
      HandleSubRuns::SubRunID const &INVALID_SUBRUNID() {
         static HandleSubRuns::SubRunID invalid_handle_subRunId(INVALID_RUN(), INVALID_SUBRUN());
         return invalid_handle_subRunId;
      }
   }

   Run::Run(art::RunNumber_t id) : id_(id) {}
   art::RunNumber_t Run::id() const { return id_; }

   SubRun::SubRun(art::SubRunNumber_t id) : id_(id) {}
   art::SubRunNumber_t SubRun::id() const { return id_; }
   ...
   art::RunNumber_t HandleRuns::currentRun() const { return currentRun_; }
   ...
   void HandleRuns::beginRun(art::RunNumber_t run) {
   ...
   void HandleRuns::endRun(art::RunNumber_t run) {
      beginRunCalled_ = false;
   ...
   HandleSubRuns::HandleSubRuns(my_context ctx) :
      ...
      currentSubRun_(INVALID_SUBRUNID()),
      previousSubRuns_(),
   ...
   void HandleSubRuns::setupCurrentSubRun() {
      ...
      art::RunNumber_t run = context<HandleRuns>().currentRun();
   ...
   bool FirstSubRun::checkInvariant() {
      ...
      assert(context<HandleSubRuns>().currentSubRun().second != INVALID_SUBRUN());
