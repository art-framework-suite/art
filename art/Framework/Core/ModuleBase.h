...
public: // MEMBER FUNCTIONS -- API for access to RandomNumberGenerator
  ...
  CLHEP::HepRandomEngine&
  createEngine(long, std::string const& kind_of_engine_to_make);

  CLHEP::HepRandomEngine&
  createEngine(long, std::string const& kind_of_engine_to_make, std::string const& engine_label);

  long
  get_seed_value(fhicl::ParameterSet const&, char const key[] = "seed", long const implicit_seed = -1);
...
public: // MEMBER FUNCTIONS -- API for using collected consumes information
  ...
  //void
  //validateConsumedProduct(BranchType const, ProductInfo::ConsumableType, std::type_info const&, std::string const& label,
...
protected: // MEMBER DATA -- For derived classes.
  ...
  std::unique_ptr<hep::concurrency::SerialTaskQueueChain>
  chain_{};
...
ProductToken<T>
ModuleBase::
consumes(InputTag const& tag)
{
  consumables_[BT].emplace_back(ProductInfo::ConsumableType::Product, TypeID{typeid(T)}, tag.label(), tag.instance(),
                                tag.process());
...
ViewToken<T>
ModuleBase::
consumesView(InputTag const& tag)
{
  consumables_[BT].emplace_back(ProductInfo::ConsumableType::ViewElement, TypeID{typeid(T)}, tag.label(), tag.instance(),
                                tag.process());
...
consumesMany()
{
  consumables_[BT].emplace_back(ProductInfo::ConsumableType::Many, TypeID{typeid(T)});
}

template <typename T, BranchType BT>
ProductToken<T>
ModuleBase::
mayConsume(InputTag const& tag)
{
  consumables_[BT].emplace_back(ProductInfo::ConsumableType::Product, TypeID{typeid(T)}, tag.label(), tag.instance(),
                                tag.process());
  return ProductToken<T>{tag};
...
ViewToken<T>
ModuleBase::
mayConsumeView(InputTag const& tag)
{
  consumables_[BT].emplace_back(ProductInfo::ConsumableType::ViewElement, TypeID{typeid(T)}, tag.label(), tag.instance(),
                                tag.process());
  return ViewToken<T>{tag};
...
//// FIXME: This is inline so that DataViewImpl which is in libart_Framework_Principal.so
//// FIXME: is not link-time dependent on libart_Framework_Core.so which contains us.
//inline
//void
//ModuleBase::
//validateConsumedProduct(BranchType const bt, ProductInfo::ConsumableType ct, std::type_info const& ti, std::string const& label,
//                        std::string const& instance, std::string const& process)
//{
