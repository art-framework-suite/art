...
string const
SharedResourcesRegistry::kLegacy{"__legacy__"};

SharedResourcesRegistry::QueueAndCounter::
~QueueAndCounter()
{
}
...
SharedResourcesRegistry::QueueAndCounter::
QueueAndCounter()
  : queue_{std::make_shared<SerialTaskQueue>()}
  , counter_{0UL}
{
}
...
void
SharedResourcesRegistry::
registerSharedResource(const string& name)
{
  // Note: This has the intended side-effect of creating the entry if it does not yet exist.
...
SharedResourcesRegistry::
createQueues(string const& resourceName) const
{
...
vector<shared_ptr<SerialTaskQueue>>
SharedResourcesRegistry::
createQueues(vector<string> const& resourceNames) const
{
  map<pair<unsigned, string>, shared_ptr<SerialTaskQueue>> sortedResources;
  if (find(resourceNames.begin(), resourceNames.end(), kLegacy) != resourceNames.end()) {
    ...
    for (auto const& keyAndVal : resourceMap_) {
      ...
      sortedResources.emplace(make_pair(queueAndCounter.counter_, key), queueAndCounter.queue_);
    }
  }
  else {
