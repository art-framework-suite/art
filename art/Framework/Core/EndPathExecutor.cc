...
endJob()
{
  ...
  // FIXME: There seems to be little value-added by the catch and rethrow here.
  for (auto& label_and_worker : endPathInfo_.workers()) {
    auto& w = label_and_worker.second;
    try {
      w->endJob();
...
{
  return any_of(outputWorkers_.cbegin(), outputWorkers_.cend(), [](auto ow) { return ow->fileIsOpen(); });
}
...
closeAllOutputFiles()
{
  for (auto ow : outputWorkers_) {
    ...
    ow->closeFile();
    actReg_.sPostCloseOutputFile.invoke(OutputFileInfo(ow->label(), ow->lastClosedFileName()));
  }
}
...
void
EndPathExecutor::
seedRunRangeSet(unique_ptr<RangeSetHandler> rsh)
{
...
{
  if (runRangeSetHandler_.at(0)->type() == RangeSetHandler::HandlerType::Open) {
    ...
    for (auto& uptr_rsh : runRangeSetHandler_) {
      //ostringstream buf;
      //buf << "-----> EndPathExecutor::setAuxiliaryRangeSetID(rp): " << idx << " " << uptr_rsh->seenRanges() << "\n";
      //try {
        mergedSeenRanges.merge(uptr_rsh->seenRanges());
      //}
      //catch (...) {
        //cerr << "-----> EndPathExecutor::setAuxiliaryRangeSetID(rp): EXCEPTION\n";
        //TDEBUG(5) << buf.str();
        //cerr << buf.str();
        //throw;
      //}
...
void
EndPathExecutor::
seedSubRunRangeSet(unique_ptr<RangeSetHandler> rsh)
{
...
{
  if (subRunRangeSetHandler_.at(0)->type() == RangeSetHandler::HandlerType::Open) {
    // We are using EmptyEvent source, need to merge
    // what the streams have seen.
    RangeSet mergedRS;
    //int idx = 0;
    for (auto& uptr_rsh : subRunRangeSetHandler_) {
      //ostringstream buf;
      //buf << "-----> EndPathExecutor::setAuxiliaryRangeSetID(srp): " << idx << " " << uptr_rsh->seenRanges() << "\n";
      //try {
        mergedRS.merge(uptr_rsh->seenRanges());
      //}
      //catch (...) {
        //cerr << "-----> EndPathExecutor::setAuxiliaryRangeSetID(srp): EXCEPTION\n";
        //TDEBUG(5) << buf.str();
        //cerr << buf.str();
        //throw;
      //}
  ...
  //
  // For a range split just before SubRun 1, Event 6, the range sets should become:
  //
  ...
  unique_ptr<RangeSetHandler> rshAtSwitch{subRunRangeSetHandler_.at(idxOfMax)->clone()};
  if (fileStatus_ == OutputFileStatus::Switching) {
    rshAtSwitch->maybeSplitRange();
    unique_ptr<RangeSetHandler> runRSHAtSwitch{runRangeSetHandler_.at(idxOfMax)->clone()};
...
void
EndPathExecutor::
process_event(EventPrincipal& ep, int si)
{
  ...
  try {
    ...
    if (action != actions::IgnoreCompletely) {
      ...
      TDEBUG(4) << "-----> End   EndPathExecutor::process_event (" << si << ") ... because of EXCEPTION\n";
      throw Exception(errors::EventProcessorFailure, "EndPathExecutor:")
          << "an exception occurred during current event processing\n"
          << ex;
    }
...
void
EndPathExecutor::
process_event(WaitingTask* finishEventTask, EventPrincipal& ep, int si)
{
  TDEBUG(4) << "-----> Begin EndPathExecutor::process_event (" << si << ") ...\n";
  ...
  endPathInfo_.incrementTotalEventCount();
  auto pathsDoneFunctor = [this, finishEventTask, &ep, si](exception_ptr const* ex) mutable {
    ...
    if (ex != nullptr) {
      try {
        ...
        auto const action{actionTable_.find(ex.root_cause())};
        if (action != actions::IgnoreCompletely) {
          auto ex_art = Exception(errors::EventProcessorFailure, "EndPathExecutor:")
              << "an exception occurred during current event processing\n"
              << ex;
          ...
          TDEBUG(4) << "=====> End   pathsDoneTask (" << si << ") ... end path because of exception\n";
          return;
        }
        // FIXME: We should not be able to get here because the only exceptions
        // FIXME: passed to the pathsDone task should be ones that terminate the path.
        mf::LogWarning(ex.category())
            << "exception being ignored for current event:\n"
            << cet::trim_right_copy(ex.what(), " \n");
        // WARNING: Processing continues below!!!
      ...
      // FIXME: We should not be able to get here because the only exceptions
      // FIXME: passed to the pathsDone task should be ones that terminate the path.
      // WARNING: We only get here if end path processing
  ...
  auto pathsDoneTask = make_waiting_task(tbb::task::allocate_root(), pathsDoneFunctor);
  ...
  try {
    ...
    if (action != actions::IgnoreCompletely) {
      ...
      // And end this task without terminating event processing
      // because our parent is the nullptr.
      TDEBUG(4) << "-----> End   EndPathExecutor::process_event (" << si << ") ... becasue of exception\n";
      return;
    }
    mf::LogWarning(ex.category())
        << "exception being ignored for current event:\n"
...
void
EndPathExecutor::
process_event_pathsDone(WaitingTask* finishEventTask, EventPrincipal& /*ep*/, int /*si*/)
{
  try {
    endPathInfo_.incrementPassedEventCount();
  }
...
void
EndPathExecutor::
writeEvent(int si, EventPrincipal& ep)
{
  for (auto ow : outputWorkers_) {
  ...
  //{
    //ostringstream buf;
    //buf << "EndPathExecutor::writeEvent: " << si << ": " << eid.run() << ", " << eid.subRun() << ", " << eid.event() << "\n";
    //cerr << buf.str();
  //}
...
//       Run that we are forced to write to finish out the file we are closing,
//       which keeps the ongoing count for closing based on SubRun and Run
//       counts meaningful).  However, the extra ones are still counted by the
//       tree entry counters.
void
EndPathExecutor::
