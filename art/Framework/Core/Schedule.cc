...
{
  TDEBUG(5) << "Schedule ctor: 0x" << hex << ((unsigned long)this) << dec << " (" << stream << ")\n";
  if (!triggerPathsInfo_.paths().empty()) {
    if ((results_inserter_ = pm.triggerResultsInserter(stream)) == nullptr) {
      ...
      WorkerParams const
        wp{process_pset_, trig_pset, outputCallbacks_, productsToProduce, actReg_, actionTable_, processName_, ModuleThreadingType::STREAM, stream};
      ModuleDescription
...
void
Schedule::
process_event(WaitingTask* endPathTask, EventPrincipal& principal, int si)
{
  ...
  auto pathsDoneFunctor = [this, endPathTask, &principal, si](exception_ptr const* ex) mutable {
    ...
    if (ex != nullptr) {
      try {
        ...
        if (action != actions::SkipEvent) {
          ...
          TDEBUG(4) << "=====> End   pathsDoneTask (" << si << ") ... trigger path processing terminate because of exception\n";
          return;
        }
        // FIXME: We should not ever be able to get here because the only exceptions
        // FIXME: passed to the pathsDone task should be ones that terminated the path.
      ...
      // FIXME: We should not ever be able to get here because the only exceptions
      // FIXME: passed to the pathsDone task should be ones that terminated the path.
      // Transfer the thrown exception to the endPath task
  ...
  try {
    ...
    if (action != actions::SkipEvent) {
      ...
      // And end this task which does not terminate event processing
      // because our parent is the nullptr.
      TDEBUG(4) << "-----> End   Schedule::process_event (" << si << ") ... because of exception\n";
      return;
    }
    mf::LogWarning(e.category())
        << "an exception occurred and all paths for the event are being skipped: \n"
...
void
Schedule::
process_event_pathsDone(WaitingTask* endPathTask, EventPrincipal& principal, int si)
{
