...
{
  oStream
    << iIndent << iPrefix << typeidToName(typeid(T)) << kNameValueSep
...
template<>
void
  doPrint<char>( std::ostream         & oStream
               , std::string    const & iPrefix
               , std::string    const & iPostfix
               , Reflex::Object const & iObject
               , std::string    const & iIndent )
{
  oStream
    << iIndent << iPrefix << "char" << kNameValueSep
...
template<>
void
  doPrint<unsigned char>( std::ostream         & oStream
                        , std::string    const & iPrefix
                        , std::string    const & iPostfix
                        , Reflex::Object const & iObject
                        , std::string    const & iIndent )
{
  ...
  oStream
    << iIndent << iPrefix << "unsigned char" << kNameValueSep
...
template<>
void
  doPrint<bool>( std::ostream         & oStream
               , std::string    const & iPrefix
               , std::string    const & iPostfix
               , Reflex::Object const & iObject
               , std::string    const & iIndent )
{
  oStream
    << iIndent << iPrefix << "bool" << kNameValueSep
...
typedef  void (*FunctionType)( std::ostream         &
                             , std::string    const &
                             , std::string    const &
                             , Reflex::Object const &
                             , std::string    const & );
...
static void
  printDataMembers( std::ostream         & oStream
                  , Reflex::Object const & iObject
                  , Reflex::Type   const & iType
                  , std::string    const & iIndent
                  , std::string    const & iIndentDelta )
{
  ...
  for( Reflex::Member_Iterator itMember = iType.DataMember_Begin()
     ; itMember != iType.DataMember_End(); ++itMember){
...
static bool
  printContentsOfStdContainer( std::ostream         & oStream
                             , std::string    const & iPrefix
                             , std::string    const & iPostfix
                             ...
                             , std::string    const & iIndentDelta )
{
  ...
  try {
    ...
    for( ;  *reinterpret_cast<bool*>(compare.Invoke(iBegin, Reflex::Tools::MakeVector((iEnd.Address()))).Address())
       ; incr.Invoke(iBegin, Reflex::Tools::MakeVector(static_cast<void*>( & dummy))), ++size ) {
  ...
  oStream
    << iPrefix << iIndent << kContainerOpen << size << "\">\n"
    << sStream.str()
    << iIndent << kContainerClose << std::endl
    << iPostfix;
  return true;
}
...
static bool
  printAsContainer( std::ostream         & oStream
                  , std::string    const & iPrefix
                  , std::string    const & iPostfix
                  , Reflex::Object const & iObject
                  , std::string    const & iIndent
  ...
  try {
    ...
    for( size_t index = 0; index != size; ++index ) {
      ...
      try {
        ...
        std::cout << iIndent << " <exception caught(" << iEx.what() << ")>\n";
      }
    }
    oStream
      << iIndent << kContainerClose << std::endl
      << iIndent << iPostfix << std::endl;
    return true;
  }
...
static void
  printObject( std::ostream & oStream
  ...
  XMLOutputModule::write(EventPrincipal const & iEP)
{
  ...
  stream_
    << "<event run=\"" << event.id().run()
    << "\" number=\"" << event.id().event() << "\" >\n";
  std::string startIndent = indentation_;
  for( Selections::const_iterator itBD = keptProducts()[InEvent].begin()
                                , itBDEnd = keptProducts()[InEvent].end()
     ; itBD != itBDEnd; ++itBD ) {
    stream_
