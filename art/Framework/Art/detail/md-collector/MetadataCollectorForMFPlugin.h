#ifndef art_Framework_Art_detail_md_collector_MetadataCollectorForMFPlugin_h
#define art_Framework_Art_detail_md_collector_MetadataCollectorForMFPlugin_h
...
namespace art {
  namespace detail {
    ...
    template<>
    class MetadataCollectorFor<suffix_type::mfPlugin> : public MetadataCollector {
    public:
    ...
    private:
      std::string header_(LibraryInfo const& li) const
      {
        ...
        result << indent_1()
               << "Destination type : " << bold_fontify(li.short_spec())
               << " (or \"" << long_spec << "\")"
               << "\n\n";
        return result.str();
      }
