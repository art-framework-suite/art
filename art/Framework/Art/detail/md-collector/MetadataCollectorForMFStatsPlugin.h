#ifndef art_Framework_Art_detail_md_collector_MetadataCollectorForMFStatsPlugin_h
...
namespace art {
  namespace detail {
    ...
    template<>
    class MetadataCollectorFor<suffix_type::mfStatsPlugin> : public MetadataCollector {
    public:
    ...
    private:
      std::string header_(LibraryInfo const& li) const
      {
        ...
        result << indent_1()
               << "Statistics destination type : " << bold_fontify(li.short_spec())
               << " (or \"" << long_spec << "\")"
               << "\n\n";
        return result.str();
      }
...
#endif /* art_Framework_Art_detail_md_collector_MetadataCollectorForMFStatsPlugin_h */
