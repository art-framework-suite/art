...
namespace art {
  namespace detail {
    ...
    template <>
    class MetadataSummaryFor<suffix_type::mfStatsPlugin> : public MetadataSummary {
    public:

      MetadataSummaryFor(LibraryInfoCollection const& coll)
        : widths_{
            std::to_string(coll.size()).size(),
            columnWidth(coll, &LibraryInfo::short_spec , "Statistics destination type"),
    ...
    private:
      ...
      std::string doHeader() const override
      {
        ...
        result << indent(widths_[0]+2)
               << std::setw(widths_[1]+4) << std::left << "Statistics destination type"
