...
detail::get_MetadataSummary(suffix_type const st, LibraryInfoCollection const& coll)
{
  switch(st) {
  ...
  case suffix_type::mfPlugin: return std::make_unique<MetadataSummaryFor<suffix_type::mfPlugin>>(coll);
  case suffix_type::mfStatsPlugin: return std::make_unique<MetadataSummaryFor<suffix_type::mfStatsPlugin>>(coll);
