...
#define DEFINE_ART_LEGACY_SERVICE_RETRIEVER(svc) \
  void* retrieve(std::shared_ptr<ServiceWrapperBase>& swb) const final override \
  { return &dynamic_cast<ServiceWrapper<svc, ServiceScope::LEGACY>*>(swb.get())->get(); }

// Global service.
#define DEFINE_ART_GLOBAL_SERVICE_RETRIEVER(svc) \
  void* retrieve(std::shared_ptr<ServiceWrapperBase>& swb) const final override \
  { return &dynamic_cast<ServiceWrapper<svc, ServiceScope::GLOBAL>*>(swb.get())->get(); }

// Legacy services.
#define DEFINE_ART_LEGACY_SERVICE_MAKER(svc) \
  std::unique_ptr<ServiceWrapperBase> make(fhicl::ParameterSet const& cfg, ActivityRegistry& reg) const final override \
  { return std::make_unique<ServiceWrapper<svc, ServiceScope::LEGACY>>(cfg, reg); }

// Global services.
#define DEFINE_ART_GLOBAL_SERVICE_MAKER(svc) \
  std::unique_ptr<ServiceWrapperBase> make(fhicl::ParameterSet const& cfg, ActivityRegistry& reg) const final override \
  { return std::make_unique<ServiceWrapper<svc, ServiceScope::GLOBAL>>(cfg, reg); }
...
#define DECLARE_ART_SERVICE_DETAIL(svc,scopeArg) \
  namespace art { \
  namespace detail { \
  template <> struct ServiceHelper<svc> : public ServiceImplHelper, public ServiceLGMHelper, public ServiceLGRHelper { \
...
#define DECLARE_ART_SERVICE_INTERFACE_DETAIL(iface,scopeArg) \
  namespace art { \
  namespace detail { \
  template <> struct ServiceHelper<iface> : public ServiceInterfaceHelper, public ServiceLGRHelper { \
...
#define DECLARE_ART_SERVICE_INTERFACE_IMPL_DETAIL(svc,iface,scopeArg) \
  namespace art { \
  namespace detail { \
  template <> struct ServiceHelper<svc> : public ServiceInterfaceImplHelper, public ServiceLGMHelper, public ServiceLGRHelper { \
    DEFINE_ART_SERVICE_TYPEID(svc) \
    DEFINE_ART_SERVICE_SCOPE(scopeArg) \
    DEFINE_ART_##scopeArg##_SERVICE_MAKER(svc) \
    DEFINE_ART_##scopeArg##_SERVICE_RETRIEVER(svc) \
    art::TypeID get_interface_typeid() const final override { return TypeID{typeid(iface)}; } \
    std::unique_ptr<ServiceWrapperBase> convert(std::shared_ptr<ServiceWrapperBase> const& swb) const final override \
    { return std::unique_ptr<art::detail::ServiceWrapperBase>( \
             static_cast<art::detail::ServiceWrapperBase*>( \
                 std::dynamic_pointer_cast<ServiceWrapper<svc, ServiceScope::scopeArg>>(swb)->getAs<iface>())); \
    } \
...
#define DECLARE_ART_SYSTEM_SERVICE_DETAIL(svc,scopeArg) \
  namespace art { \
  namespace detail { \
  template <> struct ServiceHelper<svc> : public ServiceImplHelper, public ServiceLGRHelper { \
    DEFINE_ART_SERVICE_TYPEID(svc) \
    DEFINE_ART_SERVICE_SCOPE(scopeArg) \
    bool is_interface_impl() const override { return false; } \
    DEFINE_ART_##scopeArg##_SERVICE_RETRIEVER(svc) \
...
#define DEFINE_ART_SH_CREATE(svc) \
  extern "C" { \
    ...
    create_service_helper() { return std::make_unique<art::detail::ServiceHelper<svc>>(); } \
  }

#define DEFINE_ART_SIH_CREATE(iface) \
  extern "C" { \
    std::unique_ptr<art::detail::ServiceHelperBase> \
    create_iface_helper() { return std::make_unique<art::detail::ServiceHelper<iface>>(); } \
  }
