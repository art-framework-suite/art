...
public:
  ...
  template <typename SP,
            typename = std::enable_if_t<
              detail::ServiceHelper<typename SP::element_type>::scope_val ==
              ServiceScope::PER_SCHEDULE>>
  void put(std::vector<SP>&& premade_services); // force all the services that
                                                // are not alrady made into
                                                // existance
...
private:
  ...
  std::pair<detail::ServiceCache::iterator, bool> insertImpl_(
    fhicl::ParameterSet const& pset,
    std::unique_ptr<detail::ServiceHelperBase>&& helper);

  void insertInterface_(fhicl::ParameterSet const& pset,
                        std::unique_ptr<detail::ServiceHelperBase>&& helper,
...
art::ServicesManager::put(std::vector<SP>&& premade_services)
{
  ...
  std::unique_ptr<detail::ServiceHelperBase> service_helper(
    new detail::ServiceHelper<element_type>);
  ...
  detail::WrapperBase_ptr swb(
    new detail::ServiceWrapper<element_type,
                               detail::ServiceHelper<element_type>::scope_val>(
      std::move(premade_services)));
  actualCreationOrder_.push(swb);
  factory_.emplace(
    id, detail::ServiceCacheEntry(std::move(swb), std::move(service_helper)));
}
