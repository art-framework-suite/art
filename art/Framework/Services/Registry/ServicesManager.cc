...
void
addService(string const& name, ParameterSet const& source, vector<ParameterSet>& service_set)
{
  ParameterSet tmp;
...
ServicesManager::
ServicesManager(ParameterSet&& servicesPSet, ActivityRegistry& actReg)
  : actReg_{actReg}
  ...
  , configErrMsgs_()
{
  ...
  {
    bool const wantTracer{servicesPSet.get<bool>("scheduler.wantTracer", false)};
  ...
  for (auto const& ps : psets) {
    ...
    string const service_provider{ps.get<string>("service_provider", service_name)};
    // Get the helper from the library.
    unique_ptr<detail::ServiceHelperBase> service_helper {
      lm_.getSymbolByLibspec<SHBCREATOR_t>(service_provider, "create_service_helper")()
    };
    ...
    if (service_helper->is_interface_impl()) { // Expect an interface helper
      iface_helper.reset(dynamic_cast<detail::ServiceInterfaceHelper*>(lm_.getSymbolByLibspec<SHBCREATOR_t>(service_provider, "create_iface_helper")().release()));
      if (dynamic_cast<detail::ServiceInterfaceImplHelper*>(service_helper.get())->get_interface_typeid() != iface_helper->get_typeid()) {
        throw Exception(errors::LogicError)
      ...
      if (service_provider == service_name) {
        string const iface_name {cet::demangle_symbol(iface_helper->get_typeid().name())};
    ...
    if (iface_helper) {
      ...
      factory_.emplace(iType, detail::ServiceCacheEntry(ps, move(iface_helper), svc.first->second));
    }
