...
void
art::ServicesManager::fillCache_(ParameterSets const& psets,
                                 cet::LibraryManager const& lm)
{
  ...
  for (auto const& ps : psets) {
    ...
    std::string const service_provider{
      ps.get<std::string>("service_provider", service_name)};
...
art::ServicesManager::insertImpl_(
  ...
  std::unique_ptr<detail::ServiceHelperBase>&& helper)
{
  ...
  return factory_.emplace(sType,
                          detail::ServiceCacheEntry(pset, std::move(helper)));
}

void
art::ServicesManager::insertInterface_(
  fhicl::ParameterSet const& pset,
  std::unique_ptr<detail::ServiceHelperBase>&& helper,
  detail::ServiceCache::iterator const implEntry)
{
  ...
  factory_.emplace(
    iType,
    detail::ServiceCacheEntry(pset, std::move(helper), implEntry->second));
}
// ======================================================================
