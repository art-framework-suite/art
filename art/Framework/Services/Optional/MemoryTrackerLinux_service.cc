...
namespace art {
  ...
  private:
    ...
    name_array<5u> otherInfoColumns_ {{"Step", "ModuleLabel", "ModuleType", "Vsize", "RSS"}};
    name_array<6u> eventColumns_     {{"Step", "Run", "SubRun", "Event", "Vsize", "RSS"}};
    name_array<9u> moduleColumns_   {{"Step", "Run", "SubRun", "Event", "Path", "ModuleLabel", "ModuleType", "Vsize", "RSS"}};
    name_array<11u> eventHeapColumns_ {{"Step", "Run", "SubRun", "Event", "arena", "ordblks", "keepcost", "hblkhd", "hblks", "uordblks", "fordblks"}};
    name_array<14u> moduleHeapColumns_{{"Step", "Run", "SubRun", "Event", "Path", "ModuleLabel", "ModuleType", "arena", "ordblks", "keepcost", "hblkhd", "hblks", "uordblks", "fordblks"}};
...
art::MemoryTracker::MemoryTracker(ServiceTable<Config> const& config,
  ...
  , overwriteContents_{config().dbOutput().overwrite()} // Fix so that a value of 'false' is an error if filename => in-memory db.
  ...
  , moduleHeapTable_{includeMallocInfo_ ? std::make_unique<memModuleHeap_t>(db_, "ModuleMallocInfo", moduleHeapColumns_) : nullptr}
{
  ...
  if (!fileName_.empty()) {
    iReg.sPreModuleConstruction .watch([this](auto const& md){ this->recordOtherData(md, "PreModuleConstruction"); });
    iReg.sPostModuleConstruction.watch([this](auto const& md){ this->recordOtherData(md, "PostModuleConstruction"); });
    iReg.sPreModuleBeginJob     .watch([this](auto const& md){ this->recordOtherData(md, "PreBeginJob"); });
    iReg.sPostModuleBeginJob    .watch([this](auto const& md){ this->recordOtherData(md, "PostBeginJob"); });
    iReg.sPreModuleBeginRun     .watch([this](auto const& md){ this->recordOtherData(md, "PreBeginRun"); });
    iReg.sPostModuleBeginRun    .watch([this](auto const& md){ this->recordOtherData(md, "PostBeginRun"); });
    iReg.sPreModuleBeginSubRun  .watch([this](auto const& md){ this->recordOtherData(md, "PreBeginSubRun"); });
    iReg.sPostModuleBeginSubRun .watch([this](auto const& md){ this->recordOtherData(md, "PostBeginSubRun"); });
    iReg.sPreProcessPath        .watch( this, &MemoryTracker::prePathProcessing);
    iReg.sPreProcessEvent       .watch([this](auto const& e) { this->recordEventData(e, "PreProcessEvent"); });
    iReg.sPostProcessEvent      .watch([this](auto const& e) { this->recordEventData(e, "PostProcessEvent"); });
    iReg.sPreModule             .watch([this](auto const& md){ this->recordModuleData(md, "PreProcessModule"); });
    iReg.sPostModule            .watch([this](auto const& md){ this->recordModuleData(md, "PostProcessModule"); });
    iReg.sPreWriteEvent         .watch([this](auto const& md){ this->recordModuleData(md, "PreWriteEvent"); });
    iReg.sPostWriteEvent        .watch([this](auto const& md){ this->recordModuleData(md, "PostWriteEvent"); });
    iReg.sPreModuleEndSubRun    .watch([this](auto const& md){ this->recordOtherData(md, "PreEndSubRun"); });
    iReg.sPreModuleEndRun       .watch([this](auto const& md){ this->recordOtherData(md, "PreEndRun"); });
    iReg.sPreModuleEndJob       .watch([this](auto const& md){ this->recordOtherData(md, "PreEndJob"); });
    iReg.sPostModuleEndSubRun   .watch([this](auto const& md){ this->recordOtherData(md, "PostEndSubRun"); });
    iReg.sPostModuleEndRun      .watch([this](auto const& md){ this->recordOtherData(md, "PostEndRun"); });
    iReg.sPostModuleEndJob      .watch([this](auto const& md){ this->recordOtherData(md, "PostEndJob"); });
  }
}
...
//======================================================================
void
art::MemoryTracker::recordOtherData(ModuleDescription const& md, std::string const& step)
{
  // MT-TODO: Placeholder until we're multi-threaded
  auto const sid = ScheduleID::first().id();
...
//======================================================================
void
art::MemoryTracker::recordModuleData(ModuleDescription const& md, std::string const& step)
{
  // MT-TODO: Placeholder until we're multi-threaded
  auto const sid = ScheduleID::first().id();
  auto& d = data_[sid];
...
art::MemoryTracker::flushTables_()
{
  ...
  if (eventHeapTable_) eventHeapTable_->flush();
  if (moduleHeapTable_) moduleHeapTable_->flush();
}
