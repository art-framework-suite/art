...
string const
DEFAULT_ENGINE_KIND{"HepJamesRandom"};

long constexpr
MAXIMUM_CLHEP_SEED{900000000};

long constexpr
USE_DEFAULT_SEED{-1};

art::RNGsnapshot const
EMPTY_SNAPSHOT;
...
qualify_engine_label(int const si, string const& engine_label)
{
  ...
  string label;
  label += art::PerThread::instance()->getCPC().moduleDescription()->moduleLabel();
  label += ':';
  label += to_string(si);
  label += ':';
  label += engine_label;
...
{
  return shared_ptr<CLHEP::HepRandomEngine>{(seed == USE_DEFAULT_SEED) ? new DesiredEngineType : new DesiredEngineType(seed)};
}
...
manufacture_an_engine<G4Engine>(long const seed)
{
  ...
  return shared_ptr<CLHEP::HepRandomEngine>{CLHEP::HepRandom::getTheEngine(), cet::no_delete{}};
}
...
RandomNumberGenerator(Parameters const& config, ActivityRegistry& reg)
...
CLHEP::HepRandomEngine&
RandomNumberGenerator::
getEngine(string const& engine_label) const
{
...
CLHEP::HepRandomEngine&
RandomNumberGenerator::
getEngine(int const si, string const& engine_label) const
{
...
bool
RandomNumberGenerator::
invariant_holds_(int const si)
{
  auto const& d = data_[si];
  return (d.dict_.size() == d.tracker_.size()) && (d.dict_.size() == d.kind_.size());
}

CLHEP::HepRandomEngine&
RandomNumberGenerator::
createEngine(int const si, long const seed)
{
...
CLHEP::HepRandomEngine&
RandomNumberGenerator::
createEngine(int const si, long const seed, string const& requested_engine_kind)
{
...
CLHEP::HepRandomEngine&
RandomNumberGenerator::
createEngine(int const si, long const seed, string requested_engine_kind, string const& engine_label)
{
  ...
  shared_ptr<CLHEP::HepRandomEngine> eptr{engine_factory(requested_engine_kind, seed)};
...
void
RandomNumberGenerator::
restoreSnapshot_(int const si, Event const& event)
{
  if (restoreStateLabel_.empty()) {
    return;
  }
  // access the saved-states product:
  auto const& saved = *event.getValidHandle<vector<RNGsnapshot>>(restoreStateLabel_);
  auto& d = data_[si];
...
restoreFromFile_()
{
  ...
  for (string label{}; infile >> label;) {
    ...
    if (how == EngineSource::Seed) {
      ...
      throw Exception(errors::Configuration, "RANDOM")
          << "Engine state file contains two engine states with the same label: "
          << label
          << "\n.";
    }
    else {
      throw Exception(errors::LogicError, "RANDOM")
          ...
          << label
          << " from file\n"
          << "which was originally initialized via an unknown or impossible method.\n";
    }
...
void
RandomNumberGenerator::
preProcessEvent(Event const& e)
{
  //FIXME: threading: si = 0 hardcoded here, should probably come from the module through the Event object
  takeSnapshot_(0);
