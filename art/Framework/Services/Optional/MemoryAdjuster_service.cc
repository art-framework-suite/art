...
namespace art {
  ...
  public:
    ...
    MemoryAdjuster(ServiceTable<Config> const& config [[gnu::unused]])
    {
...
#else
      auto& mopts = art::getGlobalOptionSetter();
      auto const p_mmap_max = config().m_mmap_max();
      auto const p_trim_thr = config().m_trim_thr();
      auto const p_top_pad  = config().m_top_pad();
      auto const p_mmap_thr = config().m_mmap_thr();
      ...
      if (config().dump()) {
        mf::LogWarning("MemoryCheck") << "Malloc options: " << mopts.get() << "\n";
      }
#endif
    } // c'tor
  };
