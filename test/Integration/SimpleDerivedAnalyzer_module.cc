...
SimpleDerivedAnalyzer::test_PtrVector(art::Event const & e) const
{
  ...
  test_PtrVector(art::PtrVector<arttest::Simple>(*h), event_num);
}

template <typename DATA_TYPE>
void
SimpleDerivedAnalyzer::
test_PtrVector(art::PtrVector<DATA_TYPE> const & v, int event_num) const
{
  auto sz = v.size();
  for (auto
       b = v.begin(),
       ...
       i != e; ++i) {
    int k = i - b;
    if ((unsigned)(*i)->key != sz - k + event_num) {
      std::cerr
