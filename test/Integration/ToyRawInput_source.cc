...
namespace art {
  // We don't want the file services, we must say so by specializing
  // the template.
  template<>
  struct Reader_wantFileServices<arttest::ToyFile> {
    static constexpr bool value = false;
  };
}

arttest::ToyFile::ToyFile(fhicl::ParameterSet const& ps,
...
void
arttest::ToyFile::closeCurrentFile()
{
...
bool
arttest::ToyFile::readNext(art::RunPrincipal* const& inR,
                           art::SubRunPrincipal* const& inSR,
                           art::RunPrincipal*& outR,
                           art::SubRunPrincipal*& outSR,
                           art::EventPrincipal*& outE)
...
void
arttest::ToyFile::readFile(std::string const &name,
                           art::FileBlock*& fb)
{
