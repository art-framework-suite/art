...
int main(int argc, char** argv)
{
   ...
   if (!strcmp(argv[1], "r")) {
      ...
      err = tkeyvfs_open_v2(argv[3], &db, SQLITE_OPEN_READONLY, "tkeyvfs", rootFile);
   }
   else {
      TFile* rootFile = new TFile(argv[2], "RECREATE");
      err = tkeyvfs_open_v2(argv[3], &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, "tkeyvfs", rootFile);
   }
   if (err) {
