...
fw --parameter-set ${FW_HOME}/IOPool/Common/test/PreFastMergeTest_1_cfg.py || die 'Failure using PreFastMergeTest_1_cfg.py' $?

#---------------------------
# Create first input file with extra branch
#---------------------------

fw --parameter-set ${FW_HOME}/IOPool/Common/test/PreFastMergeTest_1x_cfg.py || die 'Failure using PreFastMergeTest_1x_cfg.py' $?

#---------------------------
# Create second input file
#---------------------------

fw --parameter-set ${FW_HOME}/IOPool/Common/test/PreFastMergeTest_2_cfg.py || die 'Failure using PreFastMergeTest_2_cfg.py' $?
...
fw -j ${LOCAL_TMP_DIR}/TestFastMergeFJRx.xml --parameter-set ${FW_HOME}/IOPool/Common/test/FastMergeTest_x_cfg.py || die 'Failure using FastMergeTest_x_cfg.py' $?
#need to filter items in job report which always change
egrep -v "<GUID>|<PFN>" $FW_HOME/IOPool/Common/test/proper_fjrx_output > $LOCAL_TMP_DIR/proper_fjrx_output_filtered
...
fw -j ${LOCAL_TMP_DIR}/TestFastMergeFJR.xml --parameter-set ${FW_HOME}/IOPool/Common/test/FastMergeTest_cfg.py || die 'Failure using FastMergeTest_cfg.py' $?
#need to filter items in job report which always change
egrep -v "<GUID>|<PFN>" $FW_HOME/IOPool/Common/test/proper_fjr_output > $LOCAL_TMP_DIR/proper_fjr_output_filtered
...
fw -j ${LOCAL_TMP_DIR}/TestFastMergeRLFJR.xml --parameter-set ${FW_HOME}/IOPool/Common/test/FastMergeTestRL_cfg.py || die 'Failure using FastMergeTestRL_cfg.py' $?
#need to filter items in job report which always change
...
#need to filter items in job report which always change
egrep -v "<GUID>|<PFN>" $FW_HOME/IOPool/Common/test/proper_Rfjr_output > $LOCAL_TMP_DIR/proper_Rfjr_output_filtered
