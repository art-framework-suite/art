...
exe=${LOCAL_TMP_DIR}/src/FWCore/Framework/test/TestFrameworkCoreStatemachine/TestFrameworkCoreStatemachine
input=${FW_HOME}/FWCore/Framework/test/unit_test_outputs/statemachine_
output=statemachine_output_
reference_output=${FW_HOME}/FWCore/Framework/test/unit_test_outputs/statemachine_output_
...
pushd ${LOCAL_TMP_DIR}
  ...
  do
    ...
    ${exe} -i ${input}${i}.txt -o ${output}${i}.txt || die "TestFrameworkCoreStatemachine with input ${i}" $?
  ...
  do
    ...
    ${exe} -s -i ${input}${i}.txt -o ${output}${i}.txt || die "TestFrameworkCoreStatemachine with input ${i}" $?
    diff ${reference_output}${i}.txt ${output}${i}.txt || die "comparing ${output}${i}.txt" $?
  ...
  do
    ...
    ${exe} -m -i ${input}${i}.txt -o ${output}${i}.txt || die "TestFrameworkCoreStatemachine with input ${i}" $?
    diff ${reference_output}${i}.txt ${output}${i}.txt || die "comparing ${output}${i}.txt" $?
