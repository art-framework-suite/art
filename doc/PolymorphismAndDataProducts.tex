\documentclass[letterpaper,10pt,article]{memoir}
...
%
% Add other packages here, as needed. hyperref really wants to be
% last.
%
\usepackage{tikz-uml}
\tikzumlset{fill class=yellow!10}
\usepackage{listings}
\lstset{ basicstyle=\ttfamily % for 'normal' code
  , numbers=none % don't want line numbers
  , tabsize=2 % don't waste space with huge tabs
  , showstringspaces=false % do not use undercap as space
  , keywordstyle=\bfseries % for identified keywords
  , commentstyle=\itshape % for comments
...
\setxlvchars{\normalfont}
\setlxvchars{\normalfont}
\setlrmarginsandblock{1.0in}{*}{1.}
\setulmarginsandblock{*}{1.0in}{1.618}
\setheadfoot{2\baselineskip}{\baselineskip}
...
\newcommand{\shorttitle}{Polymorphism and Data Products}
...
\newcommand{\cpp}{C\kern-0.15ex{+}\kern-0.1ex{+}\xspace} % C++
\newenvironment{fixme}{\color{cyan}\itshape\begin{quotation}}{\end{quotation}}
...
%------------------------------------------------------------
% First page formatting is special.
%------------------------------------------------------------
\thispagestyle{empty}
\begin{flushright}
